...
Alinhe os processos with a estratégia. / Align processes with strategy.
...
Use leitura de código de barras. / Use barcode scanning.
...
Lide with remessas urgentes. / Handle urgent shipments.
...
Alinhe as melhorias with a estratégia. / Align improvements with strategy.
Executivo
Use dados para tomar decisões. / Use data to drive decisions.
...
const modules = text.split(/\d+\.\s+/).slice(1);
const levels = ["Iniciante", "Básico", "Intermediário", "Avançado", "Executivo"];
const levelMap = {
    "Iniciante": "i",
    "Básico": "b",
    "Intermediário": "m",
...
modules.forEach((moduleContent, mIdx) => {
    const mId = "m" + (mIdx + 1);
    const lines = moduleContent.trim().split('\n');
    ...
    lines.forEach(line => {
        ...
        if (currentLevel && line.indexOf(" / ") !== -1) {
            const parts = line.split(" / ");
            const pt = parts[0].trim();
            const en = parts[1].trim();
            levelCounts[currentLevel]++;
            const idx = levelCounts[currentLevel];
            const lCode = levelMap[currentLevel];
            const phraseId = mId + "-" + lCode + idx;

            const levelBase = levels.indexOf(currentLevel) * 10;
            const order = levelBase + idx;

            output.push({ id: phraseId, moduleId: mId, level: currentLevel, portuguese: pt, english: en, order: order });
        }
    });
});

fs.writeFileSync('c:/Users/inaci/OneDrive/Desktop/APPS/CogniStream/tmp_phrases.json', JSON.stringify(output, null, 2));
console.log("Success: tmp_phrases.json created.");
